...
const cacheBatchMembers = cache(
  (req) => CacheKeys.batchMembers(req.params.year, req.query.page, req.query.limit),
  60 * 60
);

// Cache posts list (15 minutes)
const cachePosts = cache(
  (req) => {
...
const cachePost = cache(
  (req) => CacheKeys.post(req.params.postId),
  30 * 60
);
...
const cacheAlumniStats = cache(
  () => CacheKeys.alumniStats(),
  4 * 60 * 60
);
