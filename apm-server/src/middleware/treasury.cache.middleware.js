...
const TreasuryCacheKeys = {
  ...
  COLLECTIONS_LIST: (filters) => `treasury:collections:${generateFilterKey(filters)}`,
  ...
  EXPORT_DATA: (type, params) => `treasury:export:${type}:${generateFilterKey(params)}`
};
...
const cacheFinancialReport = createCacheMiddleware(
  (req) => TreasuryCacheKeys.FINANCIAL_REPORT(req.params.year, req.query.format || 'json'),
 ...
 */
const invalidateSubcategoriesCache = async (categoryId, subcategoryId = null) => {
  try {
    const patterns = [
  ...
  return async (req, res, next) => {
    ...
    res.json = function(data) {
      ...
      if (data.success) {
        setImmediate(async () => {
          try {
            await invalidateFunction(...args.map(arg => 
