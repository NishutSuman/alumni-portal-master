...
class TicketMessageService {
  ...
  static async addMessage(ticketId, userId, messageData, attachments = []) {
    const { message, contentType = 'PLAIN_TEXT', formattedContent, isInternalNote = false } = messageData;
    ...
    const newMessage = await prisma.$transaction(async (tx) => {
      ...
      if (attachments.length > 0) {
        ...
        for (const attachment of attachments) {
          await TicketFileService.processFileMetadata(attachment, 'message_attachment');
        }
      }
  ...
  static async editMessage(messageId, userId, newContent, editReason = null) {
    ...
    // Create audit log
    await TicketAuditService.logAction(message.ticket.id, userId, 'MESSAGE_EDITED', {
      messageId,
      editReason
    });
  ...
  static async addReaction(messageId, userId, reactionType) {
    ...
    const hasAccess = message.ticket.userId === userId || 
    ...
    if (existingReaction) {
      ...
      // Create audit log
      await TicketAuditService.logAction(message.ticket.id, userId, 'MESSAGE_REACTION_ADDED', {
        messageId,
        reactionType
      });
  ...
  static async getMessageEditHistory(messageId, requesterId) {
    ...
    const hasAccess = message.ticket.userId === requesterId || 
