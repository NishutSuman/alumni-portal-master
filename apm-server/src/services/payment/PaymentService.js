...
class PaymentService {
  ...
  async calculateEventRegistrationTotal(registrationId) {
    try {
      ...
      // Calculate breakdown
      const registrationFee = parseFloat(registration.event.registrationFee || 0);
      const guestCount = registration.guests.length;
      const guestFees = guestCount * parseFloat(registration.event.guestFee || 0);
      
      // Calculate pending merchandise total
      const merchandiseTotal = registration.merchandiseOrders.reduce((total, order) => {
    ...
    try {
      ...
      const transaction = await prisma.paymentTransaction.create({
        data: {
          ...
          currency: 'INR',
          description: description || `Payment for ${referenceType.toLowerCase().replace('_', ' ')}`,
          referenceType,
          referenceId,
          ...
          status: 'PENDING',
          userId,
          expiresAt: new Date(Date.now() + (this.config.settings.paymentTimeout * 60 * 1000)),
  ...
  async processWebhook(provider, payload, signature) {
    try {
      ...
      const isSignatureValid = await paymentProvider.verifyWebhookSignature(payload, signature);
