...
const addToCart = async (req, res) => {
  ...
  try {
    ...
    if (merchandise.availableSizes.length > 0 && !selectedSize) {
      return errorResponse(res, 'Size selection is required for this item', 400);
    }
    ...
    if (merchandise.stockQuantity !== null && merchandise.stockQuantity < quantity) {
      return errorResponse(res, `Insufficient stock. Only ${merchandise.stockQuantity} items available`, 400);
    }
    ...
    if (existingCartItem) {
      ...
      if (merchandise.stockQuantity !== null && merchandise.stockQuantity < newQuantity) {
        return errorResponse(res, `Cannot add ${quantity} more. Stock limit exceeded. Available: ${merchandise.stockQuantity - existingCartItem.quantity}`, 400);
      }
...
const getCart = async (req, res) => {
  ...
  try {
    ...
    const cartSummary = {
      ...
      totalAmount: cartItems.reduce((sum, item) => sum + Number(item.totalPrice), 0),
    ...
    const formattedItems = cartItems.map(item => ({
      ...
      stockStatus: item.merchandise.stockQuantity !== null 
        ? (item.merchandise.stockQuantity >= item.quantity ? 'AVAILABLE' : 'INSUFFICIENT')
        : 'UNLIMITED',
...
const updateCartItem = async (req, res) => {
  ...
  try {
    ...
    if (quantity !== undefined) {
      if (cartItem.merchandise.stockQuantity !== null && cartItem.merchandise.stockQuantity < quantity) {
        return errorResponse(res, `Insufficient stock. Only ${cartItem.merchandise.stockQuantity} items available`, 400);
      }
    ...
    if (selectedSize !== undefined) {
      if (cartItem.merchandise.availableSizes.length > 0 && !cartItem.merchandise.availableSizes.includes(selectedSize)) {
        return errorResponse(res, 'Selected size is not available', 400);
      }
  ...
  try {
    ...
    for (const item of cartItems) {
      if (!item.merchandise.isActive) {
        return errorResponse(res, `Item "${item.merchandise.name}" is no longer available`, 400);
      }

      if (item.merchandise.stockQuantity !== null && item.merchandise.stockQuantity < item.quantity) {
  ...
  try {
    ...
    return successResponse(res, {
      ...
      deliveryInfo: {
        ...
        message: 'Your merchandise will be delivered manually at the event venue.'
      }
  ...
  try {
    ...
    if (search) {
      whereClause.OR = [
        ...
        { registration: { user: { fullName: { contains: search, mode: 'insensitive' } } } },
