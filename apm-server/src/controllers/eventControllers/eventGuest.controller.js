  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const updatedRegistration = await tx.eventRegistration.update({
        ...
        data: {
          ...
          paymentStatus: feeCalculation.totalAmount > registration.totalAmount ? 'PENDING' : registration.paymentStatus,
    ...
    return successResponse(res, { 
      ...
      updatedTotals: {
        totalGuests: newGuestCount,
        totalAmount: feeCalculation.totalAmount,
        additionalPaymentRequired: feeCalculation.totalAmount > registration.totalAmount,
        additionalAmount: feeCalculation.totalAmount - registration.totalAmount,
      }
  ...
  try {
    ...
    return successResponse(res, { 
      ...
      summary: {
        ...
        totalGuestFees: formattedGuests.length * (registration.event.guestFee || 0),
...
const updateGuest = async (req, res) => {
  ...
  try {
    ...
    if (!guest) {
      return errorResponse(res, 'Guest not found or does not belong to your registration', 404);
    }
    ...
    // Check modification eligibility
    const modificationCheck = eventService.canModifyRegistration(guest.registration);
    if (!modificationCheck.allowed) {
      return errorResponse(res, modificationCheck.reason, 400);
    }
    ...
    return successResponse(res, { guest: updatedGuest }, 'Guest updated successfully');
...
const submitGuestForm = async (req, res) => {
  ...
  try {
    ...
    const validation = await eventService.validateFormResponses(form.id, formResponses);
    if (!validation.valid) {
      return errorResponse(res, 'Form validation failed', 400, { errors: validation.errors });
    }
...
const getAllEventGuests = async (req, res) => {
  ...
  try {
    ...
    return paginatedResponse(res, guests, pagination, 'Event guests retrieved successfully');
