...
const registerForEvent = async (req, res) => {
  ...
  try {
        ...
        form: {
          include: {
            fields: {
              ...
              select: { id: true, fieldName: true, fieldLabel: true, isRequired: true },
            },
          },
        },
      },
    ...
    const registrationValidation = await eventService.validateEventRegistration(event, userId);
    ...
    if (event.hasMeals && !mealPreference) {
      return errorResponse(res, 'Meal preference is required for this event', 400);
    }
    
    // Validate required form fields
    if (event.form && event.form.fields.length > 0) {
      const requiredFieldIds = event.form.fields.map(field => field.id);
      const providedFieldIds = formResponses.map(response => response.fieldId);
      
      const missingFields = requiredFieldIds.filter(fieldId => !providedFieldIds.includes(fieldId));
      ...
      if (missingFields.length > 0) {
        ...
        return errorResponse(res, `Missing required fields: ${missingFieldLabels.join(', ')}`, 400);
      }
    }
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const registration = await tx.eventRegistration.create({
        data: {
          ...
          paymentStatus: feeCalculation.totalAmount > 0 ? 'PENDING' : 'COMPLETED',
...
const cancelMyRegistration = async (req, res) => {
  ...
  try {
    ...
    const modificationCheck = eventService.canModifyRegistration(registration);
    if (!modificationCheck.allowed) {
      return errorResponse(res, 'Registration cancellation deadline has passed', 400);
    }
