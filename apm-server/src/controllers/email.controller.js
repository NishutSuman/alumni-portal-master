...
const testEmailSystem = asyncHandler(async (req, res) => {
  try {
    ...
    if (testResult.success) {
      return successResponse(res, testResult, 'Email system is working correctly');
    } else {
      return errorResponse(res, `Email system test failed: ${testResult.error}`, 500);
    }
...
const sendEventReminders = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    for (const registration of event.registrations) {
      try {
        await emailService.sendEventReminder(registration.user, event, registration);
        results.push({ 
