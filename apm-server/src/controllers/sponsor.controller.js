...
const { setCacheData } = require('../middleware/sponsor.cache.middleware');
const fs = require('fs');
...
const getSponsors = async (req, res) => {
  try {
    ...
    return successResponse(res, responseData, 'Sponsors retrieved successfully');
  } catch (error) {
...
const createSponsor = async (req, res) => {
  try {
    const { name, category, description, website, contactEmail, displayOrder } = req.body;
...
const updateSponsor = async (req, res) => {
  try {
    const { sponsorId } = req.params;
    const { name, category, description, website, contactEmail, isActive, displayOrder } = req.body;
    const userId = req.user.id;
...
const deleteSponsor = async (req, res) => {
  try {
    ...
    if (sponsor.logoUrl) {
      const logoPath = sponsor.logoUrl.replace(`${req.protocol}://${req.get('host')}`, process.cwd());
      deleteUploadedFile(logoPath);
    }
    
    if (sponsor.headPhotoUrl) {
      const headPhotoPath = sponsor.headPhotoUrl.replace(`${req.protocol}://${req.get('host')}`, process.cwd());
...
const uploadSponsorLogo = async (req, res) => {
  try {
    ...
    if (currentSponsor?.logoUrl) {
      const oldLogoPath = currentSponsor.logoUrl.replace(`${req.protocol}://${req.get('host')}`, process.cwd());
...
const uploadSponsorHeadPhoto = async (req, res) => {
  try {
    ...
    if (currentSponsor?.headPhotoUrl) {
      const oldHeadPhotoPath = currentSponsor.headPhotoUrl.replace(`${req.protocol}://${req.get('host')}`, process.cwd());
...
const getSponsorStatistics = async (req, res) => {
  try {
    ...
    return successResponse(res, responseData, 'Sponsor statistics retrieved successfully');
  } catch (error) {
...
const getPublicSponsors = async (req, res) => {
  try {
    ...
    return successResponse(res, responseData, 'Public sponsors retrieved successfully');
  } catch (error) {
...
const getSponsorsByCategory = async (req, res) => {
  try {
    ...
    return successResponse(res, responseData, 'Sponsors by category retrieved successfully');
  } catch (error) {
