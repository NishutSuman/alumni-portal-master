...
const addReaction = async (req, res) => {
  try {
    ...
    const result = await TicketMessageService.addReaction(messageId, userId, reaction);
...
const getMessageEditHistory = async (req, res) => {
  try {
    ...
    const history = await TicketMessageService.getMessageEditHistory(messageId, userId);
    ...
    return errorResponse(res, error.message || 'Failed to retrieve edit history', 500);
  }
};
...
const saveMessageDraft = async (req, res) => {
  try {
    ...
    const draft = await TicketMessageService.saveMessageDraft(ticketId, userId, content);
