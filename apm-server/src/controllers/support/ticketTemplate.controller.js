const TicketTemplateService = require("../../services/ticket/ticketTemplate.service");
...
const getActiveTemplates = async (req, res) => {
	try {
		...
		const templates = await TicketTemplateService.getActiveTemplates(
			categoryId,
			userId
		);
...
const getTemplateDetails = async (req, res) => {
	try {
		...
		return errorResponse(
			res,
			error.message || "Failed to retrieve template details",
			500
		);
	}
};
...
const useTemplate = async (req, res) => {
	try {
		...
		const processedTemplate = await TicketTemplateService.useTemplate(
			templateId,
			userId
		);
		...
		return errorResponse(
			res,
			error.message || "Failed to process template",
			500
		);
	}
};
...
const deleteTemplate = async (req, res) => {
	try {
		...
		if (template.createdBy !== adminId && user.role !== "SUPER_ADMIN") {
			return errorResponse(
				res,
				"Permission denied to delete this template",
				403
			);
		}
