const TicketBulkService = require('../../services/ticketBulk.service');
...
const bulkAssignTickets = async (req, res) => {
  try {
    ...
    if (!assignedAdmin || assignedAdmin.role !== 'SUPER_ADMIN' || !assignedAdmin.isActive) {
    ...
    return errorResponse(res, error.message || 'Failed to start bulk assignment', 500);
  }
};
...
const bulkChangeStatus = async (req, res) => {
  try {
    ...
    return errorResponse(res, error.message || 'Failed to start bulk status change', 500);
  }
};
...
const bulkChangePriority = async (req, res) => {
  try {
    ...
    return errorResponse(res, error.message || 'Failed to start bulk priority change', 500);
  }
};
...
const bulkCloseTickets = async (req, res) => {
  try {
    ...
    if (!resolutionNote || resolutionNote.trim().length < 5) {
      return errorResponse(res, 'Resolution note is required (minimum 5 characters)', 400);
    }
    ...
    return successResponse(
      res,
      result,
      'Bulk close operation started',
      202
    );
  } catch (error) {
    console.error('Bulk close tickets error:', error);
    return errorResponse(res, error.message || 'Failed to start bulk close operation', 500);
  }
};
...
const bulkChangeCategory = async (req, res) => {
  try {
    ...
    return errorResponse(res, error.message || 'Failed to start bulk category change', 500);
  }
};
...
const getBulkOperationStatus = async (req, res) => {
  try {
    ...
    const operation = await TicketBulkService.getBulkOperationStatus(operationId, adminId);
    ...
    return errorResponse(res, error.message || 'Failed to get operation status', 500);
  }
};
